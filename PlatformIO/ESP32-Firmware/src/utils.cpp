...
// Remove recievePacket, pass a String packet to processPacket
...
{
    this->server = &server;  // Store the reference to avoid copying
...
{
    if (true) // (!IMU.begin()) Causing I2C errors on esp32
  {
    IMU_INITIALIZED = false;
    if (DEBUG)
    {
